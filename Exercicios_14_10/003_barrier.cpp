...
vector<pair<double, double>> resolver_bhaskaras(const vector<Equacao>& eqs) {
    ...
    #pragma omp parallel
    {
        ...
        // Espera todas as threads terminarem os deltas
        #pragma omp barrier

        // Calcular raízes usando deltas
        #pragma omp for
        for (int i = 0; i < N; i++) {
            if (deltas[i] < 0) {
...
int main() {
    ...
    /* O código utiliza OpenMP para paralelizar o cálculo das raízes de equações do 
    segundo grau com a fórmula de Bhaskara, aplicando o conceito de barreira (#pragma
    omp barrier) para sincronizar as threads entre duas fases: o cálculo do delta e o
    cálculo das raízes. O usuário informa a quantidade de equações, e o programa gera
    metade com raízes reais e metade sem. Depois, as equações são resolvidas em paralelo, 
    e o número de equações com e sem raízes é contado usando operações atômicas. O 
    objetivo é demonstrar o uso de sincronização entre threads e redução de tempo de 
    execução, mostrando que o desempenho com a barreira é semelhante ao da versão sem ela, 
    mas com melhor controle de execução paralela.
    */
