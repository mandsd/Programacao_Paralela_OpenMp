...
int main() {
    ...
    // Inicializa locks
    omp_lock_t lock_raizes, lock_nao_raizes;
    omp_init_lock(&lock_raizes);
    omp_init_lock(&lock_nao_raizes);
    ...
    for (int i = 0; i < qtd_raizes; ++i) {
        if(raizes[i].first == 0 && raizes[i].second == 0) {
            omp_set_lock(&lock_nao_raizes);
            nao_possuem_raizes++;
            omp_unset_lock(&lock_nao_raizes);
        }
        else {
            omp_set_lock(&lock_raizes);
            possuem_raizes++;
            omp_unset_lock(&lock_raizes);
        }
    }

    // Destroi locks, evitando vazamento de memória
    omp_destroy_lock(&lock_raizes);
    omp_destroy_lock(&lock_nao_raizes);
    ...
    /* O código utiliza OpenMP para resolver várias equações do segundo grau em paralelo 
    por meio da fórmula de Bhaskara. O usuário informa quantas equações serão processadas,
    e o programa gera metade com raízes reais e metade sem raízes reais. Em seguida, ele 
    calcula o delta e as raízes de todas as equações de forma paralela, utilizando locks 
    para controlar o acesso às variáveis que contam quantas possuem ou não raízes reais. 
    Por fim, o tempo total de execução é medido e exibido junto ao número de equações com 
    e sem raízes, permitindo avaliar o desempenho do paralelismo em comparação a outras 
    abordagens, como o uso de variáveis atômicas.
    */
