...
int main() {
    ...
    /*
    O código usa OpenMP para paralelizar o cálculo das raízes de equações do segundo
    grau com a fórmula de Bhaskara, permitindo resolver várias equações simultaneamente.
    O usuário informa a quantidade de equações, o programa calcula o delta e as raízes 
    em paralelo, e conta quantas têm ou não soluções reais usando operações atômicas para 
    evitar conflitos entre threads. O objetivo é mostrar o ganho de desempenho com o 
    paralelismo, reduzindo o tempo de execução de cerca de 11 segundos para 2 segundos.
    */
